...
[Stockfish Polyglot](https://stockfishchess.org) is a free, powerful UCI chess engine
derived from Glaurung 2.1 able to read up to four Polyglot book. It features
two evaluation functions, the classical evaluation based on handcrafted terms,
...
while the NNUE evaluation benefits strongly from the vector intrinsics available
on modern CPUs (avx2 or similar).

Stockfish Polyglot is not a complete chess program and requires a
...
value of the `EvalFile` UCI option, 20200720-1017.bin. This file can be downloaded from
```
https://www.comp.nus.edu.sg/~sergio-v/nnue/20200720-1017.bin
```
.
...
Currently, Stockfish Polyglot has the following UCI options:
  ...
  * #### OwnBook
    Checkbox to switch from the Polyglot book reading to the normal Stockfish searching
    functions.

  * #### BookFile
    Textboxes in which to enter the complete file path + name of the Polyglot books
    used (i.e. C:\Books\English.bin).
...
The NNUE evaluation was first introduced in shogi, and ported to Stockfish Polyglot afterward.
...
Note that the NNUE evaluation depends on the Stockfish Polyglot binary and the network parameter
file (see EvalFile). Not every parameter file is compatible with a given Stockfish Polyglot binary.
...
Stockfish Polyglot supports large pages on Linux and Windows. Large pages make
...
automatic. Stockfish Polyglot attempts to use large pages when available and
...
Stockfish Polyglot has support for 32 or 64-bit CPUs, certain hardware
instructions, big-endian machines such as Power PC, and other platforms.

On Unix-like systems, it should be easy to compile Stockfish Polyglot
...
The only real limitation is that whenever you distribute Stockfish Polyglot in
